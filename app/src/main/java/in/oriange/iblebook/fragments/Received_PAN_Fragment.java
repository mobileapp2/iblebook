...
import in.oriange.iblebook.adapters.GetReceivedPANListAdapter;
...
//        }
        ...
        if (panList.size() != 0) {
            for (int i = 0; i < panList.size(); i++) {
                if (panList.get(i).getStatus().equals("received")) {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, Bundle savedInstanceState) {
        View rootView = inflater.inflate(R.layout.fragment_received_pan, container, false);
        context = getActivity();
        init(rootView);
//        setDefault();
        getSessionData();
        ...
        protected void onPostExecute(String result) {
            ...
            try {
                ...
                if (!result.equals("")) {
                    ...
                    rv_panlist.setAdapter(new GetReceivedPANListAdapter(context, panList, "RECEIVED"));
                    if (type.equalsIgnoreCase("success")) {
                        ...
                        if (jsonarr.length() > 0) {
                            for (int i = 0; i < jsonarr.length(); i++) {
                                ...
                                if (jsonObj.getString("gst_number").equals("") && jsonObj.getString("status").equals("received")) {
